...
**Base setup:** domain `mansipandey.in`, using the WordPress REST `posts` endpoint.
...
response = requests.post(
    "https://mansipandey.in/wp-json/wp/v2/posts",
    json=payload,
    auth=auth
)
...
### 5. Security Best Practice: Environment Variables for Secrets
...
- Always store credentials/tokens in environment variables, never should be hard-coded.
